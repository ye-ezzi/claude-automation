...
def get_ig_session() -> requests.Session:
    """ig_cookies.json 파일로 인증된 Instagram 세션 반환 (싱글턴)"""
    global _ig_session
    if _ig_session is not None:
        return _ig_session
    cookie_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ig_cookies.json")
    if not os.path.exists(cookie_path):
        raise FileNotFoundError(
            f"쿠키 파일 없음: {cookie_path}\n"
            "터미널에서 1회 실행하세요: cd ~/claude-automation && python3 export_ig_cookies.py"
        )
