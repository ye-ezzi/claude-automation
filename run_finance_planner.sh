#!/bin/bash
# run_finance_planner.sh
# 매일 오전 9시 finance_reels_planner.py 실행 → 결과 이메일 발송

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TIMESTAMP=$(date '+%Y-%m-%d %H:%M:%S')
LOG_DIR="${HOME}/logs"
LOG_FILE="${LOG_DIR}/finance_planner_$(date '+%Y-%m-%d').log"

mkdir -p "$LOG_DIR"

echo "[$TIMESTAMP] 재테크 릴스 플래너 시작" >> "$LOG_FILE"
...
if [ $EXIT_CODE -ne 0 ]; then
    SUBJECT="[오류] 재테크 릴스 플래너 실패 — $(date '+%Y-%m-%d')"
    BODY="❌ finance_reels_planner.py 실행 중 오류가 발생했습니다.
